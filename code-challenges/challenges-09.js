...
const employeesBonus = (arr) => {
    ...
    arr.forEach(ele => {
        if (ele.workHours > 8) {
            let salary = ele.salary.split('$');
            salary[0] = Number(salary[0]) + 100;
            ele.salary = salary.join('$');

        } else {
            let salary = ele.salary.split('$');
            salary[0] = Number(salary[0]) + 50;
            ele.salary = salary.join('$');
        }
    })
    return arr;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    mouseArray.forEach(mouse => {
        keyBoardArray.forEach(keyboard => {
            let combination = mouse + keyboard;
            if (combination > budget) {
                combination = 0;
            } else if (combination > result) {
                result = combination;
            }
        })
    })
